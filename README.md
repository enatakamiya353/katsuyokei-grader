...
- `build_data.py` / `build_tex.py` — Ver.3.0からの出題データ生成スクリプト（参考用に同梱。
  カテゴリごとの例文プールから `random.seed` で再現可能な形にランダム抽出し、mark.tex /
  answer_key.json を生成する。サイトの動作には不要、次バージョン作成時の下敷き用）
...
- `v1.0` — 初版（20問）
- `v2.0` — 20問、同一形式で問題内容を刷新（出題カテゴリは手動でバランス配分）
- `v3.0` — 20問、出題する活用形／活用の種類を例文プールからランダム抽出する方式に変更
  （カ変「来る」は重複しすぎないよう1問に制限）
- 現行（タグなし = HEAD） — Ver.4.0（20問、同一形式・同じランダム抽出方式で、
  `build_data.py` の乱数シードのみ変えて新しい問題セットを再生成したもの）
